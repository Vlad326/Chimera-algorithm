...
def setup_detection(markers_id, markers_None):
    ...
    parameters.minMarkerPerimeterRate = 0.03  # Увеличить для скорости
    parameters.maxMarkerPerimeterRate = 0.3   # Уменьшить диапазон
    parameters.polygonalApproxAccuracyRate = 0.05  # Увеличить для скорости
    parameters.cornerRefinementMethod = cv2.aruco.CORNER_REFINE_NONE  # Отключить refinement
...
def setup_detection(markers_id, markers_None):
    ...
    # Оптимизированные параметры для скорости
    parameters.minMarkerPerimeterRate = 0.03  # Увеличить для скорости
    parameters.maxMarkerPerimeterRate = 0.4   # Уменьшить диапазон
    parameters.polygonalApproxAccuracyRate = 0.05  # Увеличить для скорости
    parameters.cornerRefinementMethod = cv2.aruco.CORNER_REFINE_NONE  # Отключить refinement
...
def rotate_angle(current_angle, change_degrees, angle_range='-180-180'):
    ...
    current_deg = np.degrees(current_angle)
...
def calc_direction_2(a, b):
    if a is not None and b is not None:
        vec = (a[0] - b[0], a[1] - b[1])
        angle_ver = np.atan2(vec[1], vec[0])
        return angle_ver
...
def get_marker_direction(marker_name, direction_angle):
    ...
    if direction_part == 'up':
        return np.radians(direction_angle + 180)
    elif direction_part == 'down':
        return np.radians(direction_angle)
    elif direction_part == 'left':
        return np.radians(direction_angle + 90)
...
def find_center_from_two_non_opposite_markers(frame, markers_centers, direction_angle):
    ...
    line1_p1 = (
        int(center1[0] - length * np.cos(direction1)),
        int(center1[1] - length * np.sin(direction1))
    )
    line1_p2 = (
        int(center1[0] + length * np.cos(direction1)),
        int(center1[1] + length * np.sin(direction1))
    ...
    line2_p1 = (
        int(center2[0] - length * np.cos(direction2)),
        int(center2[1] - length * np.sin(direction2))
    )
    line2_p2 = (
        int(center2[0] + length * np.cos(direction2)),
        int(center2[1] + length * np.sin(direction2))
    ...
    visible_length = 100
    line1_visible_end = (
        int(center1[0] + visible_length * np.cos(direction1)),
        int(center1[1] + visible_length * np.sin(direction1))
    )
    line2_visible_end = (
        int(center2[0] + visible_length * np.cos(direction2)),
        int(center2[1] + visible_length * np.sin(direction2))
    )
...
def GetResultsFrame(frame, detector, markers_id, markers_None, markers_dir, prev_time):
    # 1. Покажите оригинальное изображение
    #cv2.imshow('Original', frame)

    filtered_corners, filtered_ids, filtered_centers = None, None, None
    ...
    return {
        "markers":{"corners":filtered_corners, "ids":filtered_ids, "centers":filtered_centers},
        "robot":{'position':avg_center, 'direction':angle_deg, "side":side_text}
            }
